	...
	public class MeteorSky : CustomSky
	{
		...
		private void GenerateMeteors()
		{
			...
			for (int j = 0; j < num2; j++)
			{
				...
				_Meteors[j] = new Meteor(ModContent.Request<Texture2D>("Textures/" + paths), (float)Main.rand.NextDouble() * 4f + 6.6f);
			}
			...
			for (int i = num2; i < _Meteors.Length; i++)
			{
				...
				_Meteors[i] = new Meteor(ModContent.Request<Texture2D>("Textures/" + paths), (float)Main.rand.NextDouble() * 5f + 1.6f, 0.5f);
			}
		}
