...
namespace SpiritMod.Buffs.Pet
{
	public class SaucerPetBuff : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex)
		{
			...
			{
				Projectile.NewProjectile(player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, mod.ProjectileType("SaucerPet"), 0, 0f, player.whoAmI, 0f, 0f);
			}
		}
	}
}