...
namespace SpiritMod.Buffs
{
    public class SoulSiphon : ModBuff
	{
		...
		public override void Update(NPC npc, ref int buffIndex)
		{
			...
			{
				if (Main.player[i].active && !Main.player[i].dead && (npc.Center - Main.player[i].position).Length() < distToNPC && Main.player[i].inventory[Main.player[i].selectedItem].type == mod.ItemType("SoulSiphon") && Main.player[i].itemAnimation > 0)
				{
					if (i == Main.myPlayer)
                    {
                    ...
                    if (Main.rand.NextBool(2))
					{
						...
						int dust = Dust.NewDust(center, 1, 1, mod.DustType("SoulSiphonDust"));
