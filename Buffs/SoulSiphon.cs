    ...
    public class SoulSiphon : ModBuff
    {
        ...
        public override void Update(NPC npc, ref int buffIndex) {
            ...
            for(int i = 0; i < Main.maxPlayers; ++i) {
                if(Main.player[i].active && !Main.player[i].dead && (npc.Center - Main.player[i].position).Length() < distToNPC && Main.player[i].inventory[Main.player[i].selectedItem].type == ModContent.ItemType<SoulSiphon>() && Main.player[i].itemAnimation > 0) {
                    if(i == Main.myPlayer) {
