	...
	public class Corpsebloom : ModTile
	{
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			for(int num = 1; num <= 6; num++)
				Gore.NewGore(new EntitySource_TileBreak(i, j), new Vector2(i * 16 + Main.rand.Next(-10, 10), j * 16 + Main.rand.Next(-20, -10)), new Vector2(Main.rand.Next(-2, 2), Main.rand.Next(-2, -1)), Mod.Find<ModGore>("CorpseBloom" + num.ToString()).Type, 1f);
		}
				...
				if (distance1 < 260) {
					...
					if (++cloudtimer > 500) {
						...
						for(int num = 1; num <= 3; num++)
							Gore.NewGore(new EntitySource_TileUpdate(i, j), new Vector2(num * 16 + Main.rand.Next(-10, 10), j * 16 + Main.rand.Next(-20, -10)), new Vector2(Main.rand.Next(-2, 2), Main.rand.Next(-2, -1)), Mod.Find<ModGore>("Belch" + num).Type, 1f);
