...
namespace SpiritMod.Tiles.Furniture.NeonLights
{
	public class PurpleLightsSmall : ModTile
	{
		...
		{
			Item.NewItem(new Terraria.DataStructures.EntitySource_TileBreak(i, j), i * 16, j * 16, 48, 48, ModContent.ItemType<Items.Placeable.Furniture.Neon.PurpleFairyLights>());
		}
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            Tile tile = Framing.GetTileSafely(i, j);
            {
                ...
                Texture2D glow = ModContent.Request<Texture2D>("SpiritMod/Tiles/Furniture/NeonLights/PurpleLightsSmall_Glow", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
