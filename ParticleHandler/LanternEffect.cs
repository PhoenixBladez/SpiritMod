...
{
    public class LanternEffect : ParticleEffects
	{
		readonly static int maxtime = 3000;
		public LanternEffect() : base(new ParticleSystem("SpiritMod/ParticleHandler/Lantern", OnUpdate), 
		...
		static void OnUpdate(Particle particle)
		{
			...
			for (int i = 0; i < particle.StoredPosition.Length; i++)
				particle.StoredPosition[i] += particle.Velocity.RotatedBy(MathHelper.Pi / 4 * (float)Math.Sin(Math.PI * (particle.Timer / 60f) * (1f - particle.Scale)));


			particle.Color = Color.White * (float)Math.Sin(MathHelper.TwoPi * (particle.Timer / (float)maxtime));
			particle.Timer--;
            particle.Rotation = MathHelper.Pi / 4 * (float)Math.Sin(Math.PI * (particle.Timer / 60f) * (1f - particle.Scale))* .75f;
		}

		private static bool SpawnCondition() => Main.LocalPlayer.GetModPlayer<MyPlayer>().ZoneLantern;

		private static SpawnInfo SInfo()
		{
			Vector2 position = new Vector2(Main.rand.Next(-2000, 2000), Main.rand.Next(1200, 1600));
			return new SpawnInfo(Vector2.Zero, Main.rand.NextFloat(-2, -.6f) * Vector2.UnitY, 0f, Main.rand.NextFloat(0.5f, 1f), 
