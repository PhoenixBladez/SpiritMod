	...
	public class Ornament_Arrow : ModProjectile
	{
		...
		private void SpawnArrows()
		{
			...
			for (int i = 0; i < 1 + Main.rand.Next(5); i++) {
				...
				int a = Projectile.NewProjectile(player.Center.X + positionX, player.Center.Y + positionY, 0f, 0f, arrowType, (int)(player.HeldItem.damage * 0.7f * player.rangedDamage), 2f, player.whoAmI);
