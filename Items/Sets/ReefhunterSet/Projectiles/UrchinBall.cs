...
namespace SpiritMod.Items.Sets.ReefhunterSet.Projectiles
{
 	public class UrchinBall : ModProjectile
	{
		...
		public override void PostDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			float speed = (float)Math.Pow(projectile.timeLeft, 1.5f - (projectile.timeLeft / Cutoff));
			float alpha = 1 - (float)Math.Pow(Math.Sin(MathHelper.Pi * speed / (Cutoff * 0.25f)), 4);
