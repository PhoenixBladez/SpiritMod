		...
		public override bool PreDrawInInventory(SpriteBatch sB, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale)
		{
			for (int i = 0; i < 1; i++)
			{
				...
				for (int index2 = 0; index2 < num7; ++index2)
				{
					...
					Main.spriteBatch.Draw(Main.itemTexture[item.type], position + new Vector2(7, 8), new Microsoft.Xna.Framework.Rectangle?(r), color3, 0f, vector2_3, item.scale * .58f + num8, spriteEffects, 0.0f);
				}
			}
			return true;
		}
