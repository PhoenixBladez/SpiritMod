			...
			else
			{
				...
				if (type == mod.ProjectileType("StargloveChargePurple"))
				{
					for (float num2 = 0.0f; (double)num2 < 10; ++num2) {
						int dustIndex = Dust.NewDust(position - speed * 3, 2, 2, DustID.Clentaminator_Cyan, 0f, 0f, 0, default(Color), 1.5f);
				...
				{
					for (float num2 = 0.0f; (double)num2 < 10; ++num2) {
						int dustIndex = Dust.NewDust(position - speed * 3, 2, 2, DustID.Fire, 0f, 0f, 0, default(Color), 2f);
						Main.dust[dustIndex].noGravity = true;
