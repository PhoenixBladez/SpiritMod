...
namespace SpiritMod.Items.Sets.GreatswordSubclass.AstralSpellblade
{
    class AstralSwordPrimTrail : PrimTrail
    {
        ...
        public AstralSwordPrimTrail(Projectile projectile, Player owner, Vector2 start, Vector2 mid, Vector2 end)
        {
			Entity = projectile;
			_owner = owner;
			...
			for (float i = 0; i <= 1; i += 0.01f)
                Points.Add(Helpers.TraverseBezier(end, start, mid, i));
        }
        ...
        public override void PrimStructure(SpriteBatch spriteBatch)
        {
            ...
            float widthVar;
            float colorSin = (float)Math.Sin(Counter / 7.5f);
			...
			for (int i = 0; i < Points.Count; i++)
            {
                if (i == 0)
                {
                    ...
                    Color c1 = Color.Lerp(Color.White, new Color(67, 37, 143), colorSin);
                ...
                else
                {
                    if (i != Points.Count - 1)
                    {
                        ...
                        Color c = lightColor * ((100 - dist) * 0.01f) * (Counter / 10f);
                        Color CBT = lightColor * ((100 - dist) * 0.01f) * (Counter / 10f);
                        ...
                        AddVertex(_owner.MountedCenter + firstDown, c * AlphaValue, new Vector2((i / (float)Cap), 1));
                        AddVertex(_owner.MountedCenter + firstUp, c * AlphaValue, new Vector2((i / (float)Cap), 0));
                        AddVertex(_owner.MountedCenter + secondDown, CBT * AlphaValue, new Vector2((i + 1) / (float)Cap, 1));

                        AddVertex(_owner.MountedCenter + secondUp, CBT * AlphaValue, new Vector2((i + 1) / (float)Cap, 0));
                        AddVertex(_owner.MountedCenter + secondDown, CBT * AlphaValue, new Vector2((i + 1) / (float)Cap, 1));
                        AddVertex(_owner.MountedCenter + firstUp, c * AlphaValue, new Vector2((i / (float)Cap), 0));
                    }
                    else
        ...
        {
            PrepareShader(SpiritMod.StarfirePrims, "Edge2", Counter / 7.5f, new Color(160, 161, 225));
        }
