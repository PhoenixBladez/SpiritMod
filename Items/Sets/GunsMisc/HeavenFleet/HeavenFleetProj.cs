...
namespace SpiritMod.Items.Sets.GunsMisc.HeavenFleet
{
	...
	{
		public override void SetStaticDefaults() => DisplayName.SetDefault("Heaven Fleet");
		...
		public override bool PreAI()
		{
			...
			if (player.channel && !firing)
			{
				if (counter < 100)
				{
					...
					var dust = Dust.NewDustPerfect(dustOffset + (dustUnit * 30), 242);
			...
			else
			{
				...
				while (counter >= 0)
				{
					...
					var proj = Projectile.NewProjectileDirect(player.Center + (direction * 4), toShoot, (int)projectile.ai[0], projectile.damage, projectile.knockBack, projectile.owner);
