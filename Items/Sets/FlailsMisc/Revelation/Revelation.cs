	...
	public class RevelationProj : BaseFlailProj, ITrailProjectile
	{
		...
		public override void SpinExtras(Player player)
		{
			...
			if (++projectile.localAI[0] % 40 == 0)
			{
				...
				ParticleHandler.SpawnParticle(new RevelationParticle(projectile.Center, Vector2.Zero, Color.White, new Color(255, 94, 206), 0.7f, 20));
			}
		}

		public override void NotSpinningExtras(Player player) => projectile.localAI[0] += .5f;
		...
		public void CreateRotatingDust()
		{
			...
			for (int index1 = 0; index1 < 2; ++index1)
			{
				...
				Main.dust[index2].position = center + ((float)((double)projectile.localAI[0] / 60f * MathHelper.TwoPi + num4 * index1)).ToRotationVector2() * projectile.height;
			}
			for (int index1 = 0; index1 < 2; ++index1)
			{
				...
				Main.dust[index2].fadeIn = .85f;
				Main.dust[index2].shader = GameShaders.Armor.GetSecondaryShader(93, Main.LocalPlayer);
				Main.dust[index2].position = center + ((projectile.localAI[0] / 60f * -MathHelper.TwoPi + num4 / 2 * index1)).ToRotationVector2() * projectile.height / 1.2f;
			}
			for (int index1 = 0; index1 < 2; ++index1)
			{
				...
				Main.dust[index2].fadeIn = .85f;
				Main.dust[index2].shader = GameShaders.Armor.GetSecondaryShader(93, Main.LocalPlayer);
				Main.dust[index2].position = center + ((projectile.localAI[0] / 60f * MathHelper.TwoPi + num4 / 4f * index1)).ToRotationVector2() * projectile.height / 1.4f;
			}
		}
