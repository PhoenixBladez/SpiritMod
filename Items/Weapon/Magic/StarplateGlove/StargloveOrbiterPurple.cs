...
{
    public class StargloveOrbiterPurple : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (counter == 1)
            {
                projectile.velocity.Y = Main.rand.NextFloat(-8, 8);
                projectile.velocity.X = Main.rand.NextFloat(-8, 8);
                ColorSelect = Color.Lerp(Color.Cyan, Color.Cyan, Main.rand.NextFloat(1));
            }
            float x = 0.6f;
            float y = 0.6f;
        ...
        {
            Helpers.DrawAdditive(Helpers.RadialMask, projectile.Center.ForDraw(), ColorSelect * (FlickerFactor * 0.1f + 0.12f), Math.Abs((float)Math.Sin(counter / 120f) *0.2f) * projectile.scale);
