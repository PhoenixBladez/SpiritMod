...
namespace SpiritMod.Items.Weapon.Magic.LuminanceSeacone
{
	public class LuminanceSeaconeProjectile : ModProjectile
	{
		...
		public override void PostDraw(Color lightColor)
		{
			...
			Texture2D texture2D = ModContent.Request<Texture2D>("SpiritMod/Textures/Circle_Outline");
			if (Projectile.velocity.X == 0)
			{
				addHeight = -8f;
				addWidth = -6f;
				texture2D = ModContent.Request<Texture2D>("SpiritMod/Textures/Circle_Outline");
			}
		...
		public override void AI()
		{
			...
			if (!hasCreatedSound)
			{
				...
				SoundEngine.PlaySound(SoundID.Item, (int)Projectile.position.X, (int)Projectile.position.Y, 21, 1f, 0f);
			}
