        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack) {
            for(int i = 0; i < 3; ++i) {
                if(Main.myPlayer == player.whoAmI) {
                    ...
                    Projectile.NewProjectile(mouse.X + Main.rand.Next(-80, 80), player.Center.Y - 470 + Main.rand.Next(-50, 50), 0, Main.rand.Next(18, 28), ModContent.ProjectileType<BloodRain>(), damage, knockBack, player.whoAmI);
                }
            }
            return false;
        }
