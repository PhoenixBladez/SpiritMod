            ...
            for (int i = 0; i < Points.Count; i++)
            {
                ...
                else
                {
                    if (i != Points.Count - 1)
                    {
                        ...
                        if (_direction == 1)
                        {
                            AddVertex(firstDown + Main.player[(Entity as Projectile).owner].Center, c * AlphaValue, new Vector2((i / (float)Cap), 1));
                            AddVertex(firstUp + Main.player[(Entity as Projectile).owner].Center, c * AlphaValue, new Vector2((i / (float)Cap), 0));
                            AddVertex(secondDown + Main.player[(Entity as Projectile).owner].Center, CBT * AlphaValue, new Vector2((i + 1) / (float)Cap, 1));

                            AddVertex(secondUp + Main.player[(Entity as Projectile).owner].Center, CBT * AlphaValue, new Vector2((i + 1) / (float)Cap, 0));
                            AddVertex(secondDown + Main.player[(Entity as Projectile).owner].Center, CBT * AlphaValue, new Vector2((i + 1) / (float)Cap, 1));
                            AddVertex(firstUp + Main.player[(Entity as Projectile).owner].Center, c * AlphaValue, new Vector2((i / (float)Cap), 0));
                        }
                        else
                        {
                            AddVertex(firstDown + Main.player[(Entity as Projectile).owner].Center, c * AlphaValue, new Vector2((i / (float)Cap), 0));
                            AddVertex(firstUp + Main.player[(Entity as Projectile).owner].Center, c * AlphaValue, new Vector2((i / (float)Cap), 1));
                            AddVertex(secondDown + Main.player[(Entity as Projectile).owner].Center, CBT * AlphaValue, new Vector2((i + 1) / (float)Cap, 0));

                            AddVertex(secondUp + Main.player[(Entity as Projectile).owner].Center, CBT * AlphaValue, new Vector2((i + 1) / (float)Cap, 1));
                            AddVertex(secondDown + Main.player[(Entity as Projectile).owner].Center, CBT * AlphaValue, new Vector2((i + 1) / (float)Cap, 0));
                            AddVertex(firstUp + Main.player[(Entity as Projectile).owner].Center, c * AlphaValue, new Vector2((i / (float)Cap), 1));
                        }
                    }
