...
{
	public class Emerald_Arrow : ModProjectile
	{
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			if (Main.rand.NextBool(3)){
				int newProj = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y,
					0f, 0f,
					ModContent.ProjectileType<Projectiles.SmallExplosion>(), projectile.damage, 0, projectile.owner);
				Main.PlaySound(3, projectile.Center, 3);
				for (int index = 0; index < 16; ++index)
				{
					int i = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 89, 0.0f, 0.0f, 0, new Color(), 2.1f);
		...
		public override void Kill(int timeLeft)
		{
			...
			{
				int i = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 89, 0.0f, 0.0f, 0, new Color(), 1f);
				Main.dust[i].noGravity = true;
			}
			if (Main.rand.NextBool(3))
