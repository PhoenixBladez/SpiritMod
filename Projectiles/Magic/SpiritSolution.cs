		...
		public void Convert(int i, int j, int size = 4)
		{
			...
			for (int l = j - size; l <= j + size; l++) {
				...
				if (TileID.Sets.Conversion.Stone[type] || TileID.Sets.Conversion.Moss[type]) {
					Main.tile[k, l].TileType = (ushort)ModContent.TileType<SpiritStone>();
					WorldGen.SquareTileFrame(k, l);
					NetMessage.SendTileSquare(-1, k, l, 1);
				}

				if (type == TileID.Dirt) {
					Main.tile[k, l].TileType = (ushort)ModContent.TileType<SpiritDirt>();
					WorldGen.SquareTileFrame(k, l);
					NetMessage.SendTileSquare(-1, k, l, 1);
				}
				else if (TileID.Sets.Conversion.Grass[type]) {
					Main.tile[k, l].TileType = (ushort)ModContent.TileType<SpiritGrass>();
					WorldGen.SquareTileFrame(k, l);
					NetMessage.SendTileSquare(-1, k, l, 1);
				} 
				else if (TileID.Sets.Conversion.Sand[type]) {
					Main.tile[k, l].TileType = (ushort)ModContent.TileType<Spiritsand>();
					WorldGen.SquareTileFrame(k, l);
					NetMessage.SendTileSquare(-1, k, l, 1);
				}
				else if (TileID.Sets.Conversion.Ice[type]) {
					Main.tile[k, l].TileType = (ushort)ModContent.TileType<SpiritIce>();
					WorldGen.SquareTileFrame(k, l);
					NetMessage.SendTileSquare(-1, k, l, 1);
				}
			}
