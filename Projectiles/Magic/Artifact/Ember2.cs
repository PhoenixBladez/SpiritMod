...
{
	public class Ember2 : ModProjectile
	{
		int timer = 0;

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Frosty Ember");
		...
		public override void AI()
		{
			...
			if ((float)projectile.frameCounter >= 4f)
			{
			...
			for (int index1 = 0; index1 < 200; index1++)
			{
				...
				{
					float num23 = Main.npc[index1].position.X + (Main.npc[index1].width * .5f);
					float num24 = Main.npc[index1].position.Y + (Main.npc[index1].height * .5f);
					float num25 = Math.Abs(projectile.position.X + (projectile.width * .5f) - num23) + Math.Abs(projectile.position.Y + (float)(projectile.height * .5f) - num24);
					if (num25 < 500f)
					{
						flag25 = true;
						jim = index1;
