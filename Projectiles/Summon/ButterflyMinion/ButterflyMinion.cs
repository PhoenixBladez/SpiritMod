...
namespace SpiritMod.Projectiles.Summon.ButterflyMinion
{
	public class ButterflyMinion : ModProjectile
	{
        ...
        {
            bool flag64 = projectile.type == mod.ProjectileType("ButterflyMinion");
            Player player = Main.player[projectile.owner];
            MyPlayer modPlayer = player.GetSpiritPlayer();
            if (flag64)
            {
                if (player.dead)
                    modPlayer.butterflyMinion = false;
            ...
            {
                Dust d = Dust.NewDustPerfect(projectile.Center, 223, Vector2.One.RotatedByRandom(6.28f) * Main.rand.NextFloat(.8f), 0, default, .8f);
                d.noGravity = true;
                d.fadeIn += .75f;
            }
