	...
	class LowGravZone : ModProjectile, IDrawAdditive
	{
        ...
        {
            {
                ...
                {
                    Color color = new Color(178, 163, 191) * 0.75f * ((float)(projectile.oldPos.Length - k) / (float)projectile.oldPos.Length);

                    float scale = projectile.scale;
                    Texture2D tex = ModContent.GetTexture("SpiritMod/Projectiles/Summon/Zones/LowGravZone");
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (int k = 0; k < projectile.oldPos.Length; k++)
            {
                ...
                Microsoft.Xna.Framework.Color color29 = new Microsoft.Xna.Framework.Color(83 - projectile.alpha, 76 - projectile.alpha, 89 - projectile.alpha, 0).MultiplyRGBA(Microsoft.Xna.Framework.Color.LightBlue);
                for (int num103 = 0; num103 < 4; num103++)
                {
                    Microsoft.Xna.Framework.Color color28 = color29;
                    color28 = projectile.GetAlpha(color28);
                    color28 *= 1f - num107;
                    Vector2 vector29 = new Vector2(projectile.Center.X - 60, projectile.Center.Y - 60) + drawOrigin + ((float)num103 / (float)num108 * 6.28318548f + projectile.rotation + num106).ToRotationVector2() * (12f * num107 + 2f) - Main.screenPosition + new Vector2(0, projectile.gfxOffY) - projectile.velocity * (float)num103;
                    Main.spriteBatch.Draw(mod.GetTexture("Projectiles/Summon/Zones/LowGravZone_Glow"), vector29, new Microsoft.Xna.Framework.Rectangle?(Main.projectileTexture[projectile.type].Frame(1, Main.projFrames[projectile.type], 0, projectile.frame)), color28 * .65f, projectile.rotation, drawOrigin, projectile.scale, spriteEffects3, 0f);
                }
            }
            return false;
        }
