        ...
        {
            {
                ...
                {
                    Color color = new Color(150, 129, 35) * 0.75f * ((float)(projectile.oldPos.Length - k) / (float)projectile.oldPos.Length);

                    float scale = projectile.scale;
                    Texture2D tex = ModContent.GetTexture("SpiritMod/Projectiles/Summon/Zones/DefenseZone");
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (int k = 0; k < projectile.oldPos.Length; k++)
            {
                ...
                for (int num103 = 0; num103 < 4; num103++)
                {
                    ...
                    Main.spriteBatch.Draw(mod.GetTexture("Projectiles/Summon/Zones/DefenseZone_Glow"), vector29, new Microsoft.Xna.Framework.Rectangle?(Main.projectileTexture[projectile.type].Frame(1, Main.projFrames[projectile.type], 0, projectile.frame)), color28 * .6f, projectile.rotation, drawOrigin, projectile.scale, spriteEffects3, 0f);
                }
            }
            return false;
        }
