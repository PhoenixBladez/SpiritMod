...
namespace SpiritMod.Projectiles.Summon
{
	public class TankMinion : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (!hasTarget)
			{
				for (int i = 0; i < 200; i++)
				{
					if (Main.npc[i].CanBeChasedBy(this))
					{
						...
						var angleToTarget = Vector2.Normalize(Main.npc[i].Center - shootOffset);
						var aimAngle = angleToTarget.ToRotation();
						if (aimAngle > MathHelper.PiOver4 * 3) aimAngle -= MathHelper.Pi;
						bool inCone = Math.Abs(aimAngle) < MathHelper.PiOver4 || Math.Abs(aimAngle) > MathHelper.PiOver4 * 3;
			...
			{
				var shootOffset = projectile.Center + new Vector2(10 * projectile.direction, -8);
				var angleToTarget = Vector2.Normalize(target.Center - shootOffset);
				var aimAngle = angleToTarget.ToRotation();
				bool inCone = Math.Abs(aimAngle) < MathHelper.PiOver4 || Math.Abs(aimAngle) > MathHelper.PiOver4 * 3;
				...
				{
					phase = "moving";
					projectile.velocity.X = projectile.position.X - target.position.X > 0 ? -2 : 2;
