...
{
	public class Terror2Summon : Minion
	{
			...
			if (projectile.ai[0] == 0f && flag)
			{
				...
				if (projectile.ai[1] == 0f)
				{
					...
					if (Main.myPlayer == projectile.owner)
					{
						...
						vector2.Normalize();
						vector2 *= 9f;
						int num8 = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, vector2.X, vector2.Y, base.mod.ProjectileType("WitherShard2"), 15, projectile.knockBack, Main.myPlayer, 0f, 0f);
			...
			if (Main.rand.Next(52) == 0)
			{
				...
				if (dum2 != -1)
				{
					...
					if (flags)
					{
						...
						float dum4 = 5f;
						float dum5 = (float)Math.Sqrt(values.X * values.X + values.Y * values.Y);
						if (dum5 > dum4)
							dum5 = dum4 / dum5;

						values *= dum5;
						int p = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, values.X, values.Y, mod.ProjectileType("WitherShard1"), 22, projectile.knockBack / 2f, projectile.owner, 0f, 0f);
