		...
		public override bool PreDraw(ref Color lightColor)
		{
			if (Math.Abs(Projectile.velocity.X) > 5)
			{
				...
				for (float k = 0; k < Projectile.oldPos.Length; k+= (Projectile.oldPos.Length / 3)) {
					...
					Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Projectiles/Returning/FloraSpin", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value, drawPos, null, color, Projectile.rotation, drawOrigin, Projectile.scale, SpriteEffects.None, 0f);
				}
				return false;
			}
			return true;
