...
{
	public class HomingYikes : ModProjectile
	{
		int target;
		// USE THIS DUST: 261
		...
		{
			DisplayName.SetDefault("Wrath of Nature");
		}
		...
		public override void Kill(int timeLeft)
		{
			for (int i = 4; i < 31; i++)
			{
				...
				int newDust = Dust.NewDust(new Vector2(projectile.oldPosition.X - x, projectile.oldPosition.Y - y), 8, 8,
					107, projectile.oldVelocity.X, projectile.oldVelocity.Y, 100, default(Color), 1.8f);
				Main.dust[newDust].noGravity = true;
				Main.dust[newDust].velocity *= 0.5f;
				newDust = Dust.NewDust(new Vector2(projectile.oldPosition.X - x, projectile.oldPosition.Y - y), 8, 8,
					107, projectile.oldVelocity.X, projectile.oldVelocity.Y, 100, default(Color), 1.4f);
