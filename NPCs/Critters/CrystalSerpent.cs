...
namespace SpiritMod.NPCs.Critters
{
	public class CrystalSerpent : ModNPC
	{
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			...
			if (NPC.life <= 0)
				Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, Mod.Find<ModGore>("CrystalSerpentGore").Type, 1f);
			for (int k = 0; k < 11; k++)
		...
		public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor) => GlowmaskUtils.DrawNPCGlowMask(spriteBatch, NPC, ModContent.Request<Texture2D>("SpiritMod/NPCs/Critters/CrystalSerpent_Glow", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value, screenPos);
