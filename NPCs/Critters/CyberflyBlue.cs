...
namespace SpiritMod.NPCs.Critters
{
	public class CyberflyBlue : ModNPC
	{
		...
		public override void AI()
		{
			...
			Lighting.AddLight((int)((npc.position.X + (float)(npc.width / 2)) / 16f), (int)((npc.position.Y + (float)(npc.height / 2)) / 16f), .2f, .3f, .5f);
			if (Main.rand.Next(45) == 0)
			{
				int num622 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 226, 0f, 0f, 100, default, .5f);
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0) 
			{
				...
				{
					Dust.NewDust(npc.position, npc.width, npc.height, 226, 2.5f * hitDirection, -2.5f, 0, default, 0.57f);
					int num = Dust.NewDust(npc.position, npc.width, npc.height, 226, 0f, -2f, 0, new Color(0, 255, 142), .6f);
