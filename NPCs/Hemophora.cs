...
namespace SpiritMod.NPCs
{
    public class Hemophora : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (distance < 680)
            {
                ...
                if (npc.ai[0] == 172)
                {
                    ...
                    for (int k = 0; k < 20; k++)
                    {
                        int d = Dust.NewDust(npc.position, npc.width, npc.height, 5, -(npc.position.X - target.position.X) / distance * 2, -(npc.position.Y - target.position.Y) / distance * 4, 0, default(Color), Main.rand.NextFloat(.65f, .85f));
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            ...
            return spawnInfo.spawnTileY > Main.rockLayer ? 0.0368f : 0f;
        }
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            for (int k = 0; k < 20; k++)
            {
                int d = Dust.NewDust(npc.position, npc.width, npc.height, 5, hitDirection * 2.5f, -1f, 0, default(Color), Main.rand.NextFloat(.45f, .95f));
                Main.dust[d].fadeIn = 1f;
                Main.dust[d].noGravity = true;
            }
        }
