...
namespace SpiritMod.NPCs.CrimsonWorm

{
	public class CrimsonWorm1 : ModNPC
	{
		...
		public override bool PreAI()
		{
			if (Main.netMode != 1)
			{
				if (npc.ai[0] == 0)
				{
					...
					latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("CrimsonWorm3"), npc.whoAmI, 0, latestNPC);
					Main.npc[(int)latestNPC].realLife = npc.whoAmI;
					Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
