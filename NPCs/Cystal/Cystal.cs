	...
	public class Cystal : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (npc.active && player.active && !shieldSpawned)
			{
				...
				Main.npc[shield1].Center += new Vector2(0, 200).RotatedBy(MathHelper.ToRadians(90));
				...
				Main.npc[shield2].Center += new Vector2(0, 200).RotatedBy(MathHelper.ToRadians(180));
				...
				Main.npc[shield3].Center += new Vector2(0, 200).RotatedBy(MathHelper.ToRadians(270));
				...
				Main.npc[shield4].Center += new Vector2(0, 200).RotatedBy(MathHelper.ToRadians(360));
				Main.npc[shield4].ai[1] = npc.whoAmI;
			}
			if ((double)Vector2.Distance(npc.Center, player.Center) < (double)200f)
			{
			...
			{
				NPC npc2 = Main.npc[i];
				if ((double)Vector2.Distance(npc.Center, npc2.Center) < (double)200f && healTimer % 60 == 0 && npc2.type != mod.NPCType("Cystal") && npc2.type != mod.NPCType("Cystal_Shield"))
				{
			...
			if ((double)Vector2.Distance(player.Center, npc.Center) <= (double)200f && (double)Vector2.Distance(player.Center, npc.Center) > (double)100f)
			{
				npc.velocity = Vector2.Zero;
			}

			if ((double)Vector2.Distance(player.Center, npc.Center) > (double)200f && player.active)
			{
				float num1 = 4f;
				float number2 = 0.25f;
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			...
			for (int k = 0; k < 7; k++)
			{
				Dust.NewDust(npc.position, npc.width, npc.height, 184, 2.5f * hitDirection, -2.5f, 0, default(Color), 1.2f);
				Dust.NewDust(npc.position, npc.width, npc.height, 184, 2.5f * hitDirection, -2.5f, 0, default(Color), 0.5f);
				Dust.NewDust(npc.position, npc.width, npc.height, 70, 2.5f * hitDirection, -2.5f, 0, default(Color), 0.7f);
			}
		}
