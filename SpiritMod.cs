...
namespace SpiritMod
{
	public class SpiritMod : Mod
	{
		...
		public override void UpdateMusic(ref int music, ref MusicPriority priority)
		{
			...
			if (Main.gameMenu || priority > MusicPriority.Event || !player.active)
				return;
			...
			if (spirit.ZoneSpirit && NPC.downedMechBossAny)
			{
				...
				else if (player.position.Y / 16 >= Main.maxTilesY - 330)
		...
		{
			if (Main.LocalPlayer.GetModPlayer<MyPlayer>().ZoneReach && !Main.dayTime)
				scale *= .95f;
		}
		...
		internal static string GetWeatherRadioText(string key)
		{
			...
			if (MyWorld.aurora) return "Aurora";
			if (MyWorld.BlueMoon) return "Mystic Moon";
			if (MyWorld.jellySky) return "Jelly Deluge";
			if (MyWorld.luminousOcean) return "Luminous Seas";
			if (MyWorld.calmNight) return "Calm Might";
			if (MyWorld.rareStarfallEvent) return "Starfall";
		...
		public override void ModifyInterfaceLayers(List<GameInterfaceLayer> layers)
		{
			...
			if (TideWorld.TheTide)
			{
				...
				var NewLayer = new LegacyGameInterfaceLayer("InstallFix2: NewLayer",
					delegate
					{
		...
		internal void DrawUpdateToggles()
		{
			...
			var AutoSellUI_TOGGLERECTANGLE = new Rectangle(494, 312, 39, 39);
			...
			var Sell_NoValue_TOGGLERECTANGLE = new Rectangle(502, 432, 32, 32);
			...
			var Sell_Lock_TOGGLERECTANGLE = new Rectangle(502, 356, 32, 32);
			...
			var Sell_Weapons_TOGGLERECTANGLE = new Rectangle(502, 394, 32, 32);
