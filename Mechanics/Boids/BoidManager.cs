	...
	public class BoidHost
	{
		...
		public void LoadContent()
		{
			...
			for (int i = 0; i < flocks; i++)
			{
				...
				Texture2D[] sardineTexture = new Texture2D[] { ModContent.Request<Texture2D>($"SpiritMod/Textures/AmbientFish/sardine", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value };
				Texture2D[] shrimpTexture = new Texture2D[] { ModContent.Request<Texture2D>($"SpiritMod/Textures/AmbientFish/shrimp", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value };
				...
				for (int j = 0; j < textures.Length; ++j)
				{
					...
					{
						textures[j] = ModContent.Request<Texture2D>($"SpiritMod/Textures/AmbientFish/fish_{id}", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
						addedIDs[id] = true;
					}
					else
						j--;
