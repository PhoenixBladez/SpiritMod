...
namespace SpiritMod.Mechanics.QuestSystem.Quests
{
    public class StylistQuestSeafoam : Quest
    {
		...
		{
			if (pool[ModContent.NPCType<NPCs.SeaMandrake.SeaMandrake>()] > 0f && !NPC.AnyNPCs(ModContent.NPCType<NPCs.SeaMandrake.SeaMandrake>()))
			{
				pool[ModContent.NPCType<NPCs.SeaMandrake.SeaMandrake>()] = 0.15f;
			}
		}
		private StylistQuestSeafoam()
        {
			_tasks.AddParallelTasks(new SlayTask(ModContent.NPCType<NPCs.SeaMandrake.SeaMandrake>(), 1), new RetrievalTask(ModContent.ItemType<Items.Sets.MaterialsMisc.QuestItems.SeaMandrakeSac>(), 1, "Harvest"))
