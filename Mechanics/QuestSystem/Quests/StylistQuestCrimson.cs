...
namespace SpiritMod.Mechanics.QuestSystem.Quests
{
    public class StylistQuestCrimson : Quest
    {
		...
		{
			if (npc.type == ModContent.NPCType<NPCs.ArterialGrasper.CrimsonTrapper>())
			{
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.Sets.MaterialsMisc.QuestItems.CrimsonDyeMaterial>());
			}
		}
		private void QuestGlobalNPC_OnEditSpawnPool(IDictionary<int, float> pool, NPCSpawnInfo spawnInfo)
		{
			if (pool[ModContent.NPCType<NPCs.ArterialGrasper.CrimsonTrapper>()] > 0f && !NPC.AnyNPCs(ModContent.NPCType<NPCs.ArterialGrasper.CrimsonTrapper>()))
			{
				pool[ModContent.NPCType<NPCs.ArterialGrasper.CrimsonTrapper>()] = 0.185f;
			}
		}
		private StylistQuestCrimson()
        {
			_tasks.AddParallelTasks(new SlayTask(ModContent.NPCType<NPCs.ArterialGrasper.CrimsonTrapper>(), 1, "ArterialGrasper"), new RetrievalTask(ModContent.ItemType<Items.Sets.MaterialsMisc.QuestItems.CrimsonDyeMaterial>(), 1, "Harvest"));
			
			TaskBuilder branch1 = new TaskBuilder();
			branch1.AddTask(new GiveNPCTask(NPCID.Stylist, ModContent.ItemType<Items.Sets.MaterialsMisc.QuestItems.CrimsonDyeMaterial>(), 1, "Wow, you smell pretty terrible. Take a shower, then come back to me for a shampoo on the house. I insist! But thank you for doing this for me, hun. You've been working really hard, so I wanted to give you this exclusive hair dye I've creates. You'll never find it anywhere else! This dye and the one you've helped create work best with white hair, so let me know if you want a pre-dye.", "Bring the Violet Crystal back to the Stylist", true, true, ModContent.ItemType<Items.Sets.DyesMisc.HairDye.CystalDye>()));
